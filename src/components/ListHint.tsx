...
function ListHint({ hints }: { hints: string[] }) {
    ...
    return (
        <AppLayout flexDirection={"column"} maxW="lg">
            <Stack justifyContent="center" width="100%" py={20}>
                <Box minH="500">
                    <Text textAlign="center" color="#A680FF" fontSize="3xl" fontWeight="medium" textShadow="0px 4px 11px rgba(0, 0, 0, 0.25)" mb={10}>
                        LIST OF HINTS
                    </Text>
                    <VStack gap={3}>
                        ...
                        <PrimaryButton onClick={() => SubmitHints().finally(off)} isLoading={isLoading}>
                            Submit
                        </PrimaryButton>
                    </VStack>
                </Box>
