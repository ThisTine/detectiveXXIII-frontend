  ...
  return (
    <HStack w="100%" pos="fixed" top={0} justifyContent="flex-end" >
      ...
      {NavBarData.map(({Component,to,title})=><ButtonIcon to={to} title={title} key={title} > <Component/>  </ButtonIcon>)}
        ...
        <Button w="100%" h="100%" bg="white" onClick={logout} gap={2} p={2} px={3} shadow="lg" rounded="2xl"> <Tooltip label="Logout"><Heading size={"lg"} color="#F56565" > <AiOutlineLogout/> </Heading></Tooltip> </Button>
